...
const uint8_t AdcChannels[] = ADC_CHANNELS;
...
void Adc_t::Init() {
    rccEnableADC1(FALSE);   	// Enable digital clock
    Clk.EnableHSI();            // ADC uses HSI as clock source
    ...
    for(uint8_t i=0; i < ADC_CHANNEL_CNT; i++) {
		...
		for(uint8_t j=0; j<ADC_OVERSAMPLING_RATIO; j++) SetSequenceItem(SeqIndx++, AdcChannels[i]);
	}
    // ==== DMA ====
    dmaStreamAllocate     (ADC_DMA, IRQ_PRIO_LOW, AdcTxIrq, NULL);
    dmaStreamSetPeripheral(ADC_DMA, &ADC1->DR);
...
#endif
    // Find bounds
    uint32_t Start = Indx * ADC_OVERSAMPLING_RATIO;
    uint32_t Stop  = Start + ADC_OVERSAMPLING_RATIO;
    // Average values
    uint32_t Rslt = 0;
    for(uint32_t i = Start; i < Stop; i++) Rslt += IBuf[i];
    return Rslt / ADC_OVERSAMPLING_RATIO;
}
#endif // f4xx & L151
